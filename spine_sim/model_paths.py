...
def apply_calibration(base_model: SpineModel, params: dict) -> SpineModel:
    ...
    if B > 0:
        ratios = np.array([float(params.get(f"maxwell_k_ratio_{b}", 0.0)) for b in range(B)], dtype=float)
        tau_ms = np.array([float(params.get(f"maxwell_tau_ms_{b}", 0.0)) for b in range(B)], dtype=float)
    ...
    limit_m = None if base_model.compression_limit_m is None else base_model.compression_limit_m.copy()
    if limit_m is None:
...
def calibrate_peaks(
    ...
    *,
    ...
    """
    Defaults tuned for your runtime:
      - exploration on ALL 5 cases is expensive, so keep explore_samples modest,
        then rely on multiple diverse refinements.
    """
    return calibrate_model_peaks_joint(
        ...
        explore_samples=60,
        explore_keep=30,
        explore_local_frac=0.6,
        explore_local_sigma=0.20,
        diversity_min_dist=0.35,
        snap_norm_step_explore=0.01,
        snap_norm_step_refine=0.001,
        cache_norm_step_model=0.01,
        explore_max_newton_iter=12,
        explore_newton_tol=1e-7,
        refine_max_newton_iter=25,
        refine_newton_tol=1e-9,
    )
